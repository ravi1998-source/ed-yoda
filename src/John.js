import React from 'react'
import './John.css'
import LinkedInIcon from '@material-ui/icons/LinkedIn';
import LanguageIcon from '@material-ui/icons/Language';
import GitHubIcon from '@material-ui/icons/GitHub';
import Expertise from './Expertise.js'


function John() {
    return (
        <div className="john">
           <div className="john__box">
            <div className="john__inner">
                <div className="john__innerel">
                <img
                 className="https://image.shutterstock.com/image-vector/cute-angry-monster-on-yellow-600w-1020286750.jpg"
                 />
                 <ol>
                     <li><LinkedInIcon /></li>
                     <li><LanguageIcon /></li>
                     <li><GitHubIcon/></li>
                 </ol> 
                </div>
                <div className="john__name">
                    <h1>John Doe</h1>
                </div>
                <div className="john__image">
                    <img src="https://ak.picdn.net/shutterstock/videos/1011640655/thumb/11.jpg" />
                </div>
                <div className="data1">
                    <h4>Stats</h4>
                    <h5>1320</h5>
                    <p>Total Hours Taught</p>
                </div>
                <div className="data2">
                  <h5>468</h5>
                  <p>Student Taught</p>
                </div>
                <div className="technichal">
                <Expertise loc="data:image/png;base64,"
                 title="HTML"    
                 />
                 <Expertise loc="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1200px-CSS3_logo_and_wordmark.svg.png"
                  title="CSS"  
                  /> 
                <Expertise loc="https://wp.sitepen.com/wp-content/uploads/2016/04/js.png"
                 title="Javascript"
                 />              
                 </div>

            </div>
           </div> 
        </div>
    )
}

export default John
